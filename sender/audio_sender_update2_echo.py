...
SEND_AUDIO_CMD = (
    f"gst-launch-1.0 -v alsasrc device=hw:3,0 ! audioconvert ! audioresample ! "
    f"audio/x-raw,format=S16BE,channels=2,rate=32000 ! rtpL16pay ! "
    f"srtsink uri=\"srt://{SRT_SERVER}:{SEND_PORT}?mode=caller\""
)

# ✅ Fixed: Removed unnecessary line breaks
RECEIVE_AUDIO_CMD = (
    f"gst-launch-1.0 -v srtsrc uri=\"srt://{SRT_SERVER}:{RECEIVE_PORT}?mode=caller&latency=1000&maxbw=0\" ! "
    f"queue max-size-time=200000000 ! application/x-rtp,media=audio,clock-rate=32000,encoding-name=L16,channels=2 ! "
    f"rtpL16depay ! audioconvert ! audioresample ! webrtcechoprobe ! alsasink device=hw:2,0 sync=false"
)
...
if __name__ == "__main__":
    print("Starting audio send & receive pipelines with echo cancellation...")
