...
def build_pipeline(cfg):
    ...
    srt_audio_src_2 = srt_cfg.get("audio_src_2", "srt://:8803?mode=listener")
    srt_audio_sink = srt_cfg.get("audio_sink", "srt://:8802?mode=listener")

    pipeline_str = f"""
    srtsrc uri="{srt_video_src}" ! queue ! srtsink uri="{srt_video_sink}"
    srtsrc uri="{srt_audio_src}" ! queue ! application/x-rtp,media=audio,clock-rate=32000,encoding-name=L16,channels=2 ! rtpL16depay ! audioconvert ! audioresample ! queue ! mixer. \
    srtsrc uri="{srt_audio_src_2}" ! queue ! application/x-rtp,media=audio,clock-rate=32000,encoding-name=L16,channels=2 ! rtpL16depay ! audioconvert ! audioresample ! queue ! mixer. \
    audiomixer name=mixer ! audioconvert ! audioresample ! audio/x-raw,format=S16BE,channels=2,rate=32000 ! rtpL16pay ! srtsink uri="{srt_audio_sink}" wait-for-connection=false
    """

    return pipeline_str, srt_audio_src, srt_audio_src_2, srt_audio_sink
...
def get_audio_recording_pipeline():
    ...
    pipeline_str = f"""
    srtsrc uri="srt://:8802?mode=listener" ! queue !
...
def main():
    global loop
    cfg = load_config()
    pipeline_str, srt_audio_src, srt_audio_src_2, srt_audio_sink = build_pipeline(cfg)
