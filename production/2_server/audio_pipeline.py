...
def build_audio_pipeline():
    pipeline = f"""
        ...
        tee_tn. ! queue !
          ...
          rtpL16pay !
          srtsink uri="srt://:{audio_loopback_dk}?mode=listener" wait-for-connection=false keep-listening=true
        ...
        tee_dk. ! queue !
          audioconvert ! audioresample ! audio/x-raw,channels=1,rate=32000 !
          rtpL16pay !
          srtsink uri="srt://:{audio_loopback_tn}?mode=listener" wait-for-connection=false keep-listening=true
