...
def build_video_pipeline():
    ...
    pipeline1 = (
        f'srtsrc name=v_send_tn uri="srt://0.0.0.0:{video_send_tn}?mode=listener&latency=100" '
        f'! queue max-size-time=2000000000 max-size-buffers=200 '
        f'! tee name=tee_tn '
        f' tee_tn. ! queue ! srtsink name=v_recv_dk uri="srt://0.0.0.0:{video_receive_dk}?mode=listener&latency=100"'   
    )
    pipeline2 = (
        f'srtsrc name=v_send_dk uri="srt://0.0.0.0:{video_send_dk}?mode=listener&latency=100" '
        f'! queue max-size-time=2000000000 max-size-buffers=200 '
        f'! tee name=tee_dk '
        f' tee_dk. ! queue ! srtsink name=v_recv_tn uri="srt://0.0.0.0:{video_receive_tn}?mode=listener&latency=100"'  
    )
